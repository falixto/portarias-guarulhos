import requests
from bs4 import BeautifulSoup
import re
import csv
import time
import pdfplumber
from io import BytesIO

BASE_URL = "https://www.guarulhos.sp.leg.br"
LISTA_URL = BASE_URL + "/documentos/tipo:leis-municipais-3/subtipo:portaria-8"

headers = {
    "User-Agent": "Mozilla/5.0"
}

def extrair_matricula(texto):
    m = re.search(r'matr[ií]cula\s*(?:nº|n\.|:)?\s*(\d+)', texto, re.IGNORECASE)
    if not m:
        m = re.search(r'c[oó]d\.?\s*(\d+)', texto, re.IGNORECASE)
    return m.group(1) if m else ""

def identificar_tipo(texto):
    texto = texto.upper()
    for termo in ["CONCEDE", "NOMEIA", "EXONERA", "DESIGNA", "PRORROGA", "REVOGA"]:
        if termo in texto:
            return termo.capitalize()
    return "Outro"

def extrair_texto_pdf(pdf_url):
    pdf_bytes = requests.get(pdf_url, headers=headers).content
    texto = ""
    with pdfplumber.open(BytesIO(pdf_bytes)) as pdf:
        for pagina in pdf.pages:
            texto += pagina.extract_text() or ""
    return texto

def main():
    response = requests.get(LISTA_URL, headers=headers)
    soup = BeautifulSoup(response.text, "html.parser")

    registros = []

    # Cada portaria está em um bloco <article>
    artigos = soup.find_all("article")

    for art in artigos:
        h2 = art.find("h2")
        if not h2:
            continue

        a = h2.find("a")
        if not a:
            continue

        portaria = a.text.strip()
        pagina_url = a["href"]

        texto_artigo = art.get_text(" ", strip=True)

        data_match = re.search(r'Publicado:\s*(\d{2}/\d{2}/\d{4})', texto_artigo)
        data = data_match.group(1) if data_match else ""

        # Abre página individual
        pagina_html = requests.get(pagina_url, headers=headers).text
        pagina_soup = BeautifulSoup(pagina_html, "html.parser")

        pdf_link = pagina_soup.find("a", href=re.compile(r"\.pdf"))
        if not pdf_link:
            continue

        pdf_url = pdf_link["href"]

        texto_pdf = extrair_texto_pdf(pdf_url)

        matricula = extrair_matricula(texto_pdf)
        tipo = identificar_tipo(texto_pdf)

        assunto = texto_pdf.split(".")[0].strip()

        registros.append([
            portaria,
            matricula,
            tipo,
            data,
            assunto
        ])

        time.sleep(1)

    with open("portarias_garulhos.csv", "w", newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f, delimiter=";")
        writer.writerow(["Portaria", "Matricula", "Tipo", "Data", "Assunto"])
        writer.writerows(registros)

if __name__ == "__main__":
    main()
